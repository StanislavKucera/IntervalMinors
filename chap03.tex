...
\begin{proof}
\begin{itemize}
	\item[$\Rightarrow$] Let $P_1\in\bin^{k_1\times l_1}$ and $P_2\in\bin^{k_2\times l_2}$. While $P$ is an interval minor of neither $P_1\dsum_{1\times1}\{0\}^{k_2\times l_2}$ nor $\{0\}^{k_1\times l_1}\dsum_{1\times1}P_2$, we have that $P\im P_1\dsum_{1\times1}\{0\}^{k_2\times l_2}\dsum\{0\}^{k_1\times l_1}\dsum_{1\times1}P_2$.
	\item[$\Leftarrow$] Let there be non-empty matrices $M_1,M_2\in\Avm{\class{P}}$ such that there is a minimal $P\in\class{P}$ of size $k\times l$ for which $P\im M=M_1\dsum_{1\times1}M_2$. From Observation~\ref{obs:keep} there exists an articulation~$P[r,c]$ and both submatrices~$P[[r,k],[c]],P[[r],[c,l]]$ are non-empty.
	
	It remains to show that assuming $M_1$ and $M_2$ are non-empty is without loss of generality. If $\Avm{\class{P}}$ is non-trivial, it contains a non-empty matrix~$M$. We can keep applying the skew sum with $1\times1$ overlap to $M$ until we either find $M_1,M_2$ non-empty such that $M_1\dsum_{1\times1}M_2\not\in\Avm{\class{P}}$ or create a big enough non-empty matrix such that for any empty $M'\in\Avm{P}$ it holds $M'\im M$. 
	
	This does not work only if $M=\{1\}^{1\times1}$ and it is the only non-empty matrix in $\Avm{\class{P}}$. But then consider the matrix~$M'$ that is equal to the skew sum with $1\times1$ overlap of $M$ and any non-empty matrix $M''\in\Avm{\class{P}}$ of size bigger than $1\times1$ (the class is non-trivial so there is one). There is a pattern~$P\in\class{P}$ such that $P\im M'$ and because $P\nim M''$, $P\in\Pat$ contains exactly one one-entry~$P[r,c]$. It means that $P[r,c]$ is an articulation and $P$ is the skew sum with $1\times1$ overlap of its non-empty interval minors, as $P=P[[r,k],[c]]\dsum_{1\times1}P[[r],[c,l]]$. \qedhere
\end{itemize}
\end{proof}
...
\begin{proof}
\begin{itemize}
	\item[$\Rightarrow$] Having Proposition~\ref{prop:0to1} in mind, consider the skew sum with $1\times1$ overlap of multiple copies of elements of $\class{P}$ and let the sequence contain an articulation between each pair of consecutive copies of matrices from $P$, together with the trivial articulations~$M[m,1]$ and $M[1,n]$.
...
\end{thm}
\begin{proof}
Let $\class{F}$ be the set of all minimal (with respect to interval minors) matrices such that $\Cl{M}=\Avm{\class{F}}$. We need to prove that $\class{F}$ is finite. Thanks to Proposition~\ref{prop:0to1}, $\Avm{\class{F}}$ is closed under the direct sum with $1\times1$ overlap and from Lemma~\ref{lemma:rel} follows that for no $F\in\class{F}$ there are its non-empty interval minors $F_1,F_2$ such that $F=F_1\dsum{1\times1}F_2$. We denote by $\class{P}$ a set of matrices $F\in\class{F}$ such that $F$ has at most $2m+4$ rows and $2n+4$ columns. We want to show $\Cl{M}=\Avm{\class{P}}$.
\begin{itemize}
